#!/usr/bin/env python3
"""DTV vs Naive A/B experiment visualization -- OLD 100-case data.

Data source: result/ab_merged_results.json (100 cases, budget=6144 fixed, do_sample=on)
Output: plots_old/
...
OUT_DIR = Path("plots_old")
OUT_DIR.mkdir(exist_ok=True)

with open("result/ab_merged_results.json") as f:
...
BUDGET = 6144  # old data uses fixed budget

with open("result/c_token_counts_old.json") as f:
...
ax1.set_title(
    f"OLD {N_TOTAL}-Case A/B (budget=6144): DTV pass={dtv_pct:.1f}% vs Naive pass={naive_pct:.1f}%",
...
add_tldr(fig1,
    ...
    "Budget=6144 fixed, do_sample=on.")
...
ax2a.set_title("OLD: Cat1 (Both Pass) Token Ratio by Sub-Category", fontsize=14, fontweight="bold")
...
if cat1d:
    ...
    add_tldr(fig2a,
        f"TL;DR: Both fix (Cat1d, {len(cat1d)} cases) -> DTV {cat1d_pct:.0f}% {cat1d_dir} (median={cat1d_med:.2f}). "
        f"Budget=6144 fixed.")
else:
    add_tldr(fig2a, "TL;DR: No Cat1d cases in old data.")
...
n_cat1 = len(cat1_all)
ax6a.set_title(f"OLD: CDF Normalized Token Usage (Cat1 Both Pass, N={n_cat1})", fontsize=14, fontweight="bold")
ax6a.legend(fontsize=12, loc="lower right")
budget_ks = [BUDGET / c_tokens[c] for c in cat1_all]
min_bk = min(budget_ks)
ax6a.set_xlim(0, min_bk)
...
fig6a.tight_layout()
add_tldr(fig6a,
    f"TL;DR: Cat1 N={n_cat1}. DTV median k={dtv_med_k:.1f} vs "
    f"Naive k={naive_med_k:.1f}. Wins {wins}:{losses}:{ties}. "
    f"Sign p={sign_p:.3f}, Wilcoxon p={wil_p:.3f}. Budget=6144 fixed.")
...
if cat3:
    ...
    ax4b.set_title(f"[Internal] OLD: Cat3 Naive Passes, DTV Fails ({len(cat3)} cases)", fontsize=13, fontweight="bold")
    ...
    print("Skipped fig4b_cat3_reverse.png (no Cat3 cases)")
...
ax6b.set_title(f"[Internal] OLD: CDF Normalized Token Usage - All {N_TOTAL} Cases", fontsize=14, fontweight="bold")
ax6b.legend(fontsize=12, loc="lower right")
ax6b.set_xlim(0, None)

# Budget region (varies per case since budget=6144 is fixed)
all_budget_ks = [BUDGET / c_tokens[c] for c in all_cids]
med_bk_all = float(np.median(all_budget_ks))
ax6b.axvline(med_bk_all, color="black", linestyle="-", linewidth=1, alpha=0.3)
ax6b.text(med_bk_all, 0.05, f"budget median\nk={med_bk_all:.1f}", ha="right", fontsize=8, alpha=0.5, rotation=90)
...
ax8.set_title(f"[Internal] OLD: Token Ratio Distribution (Cat1, N={len(cat1_all)})", fontsize=14, fontweight="bold")
ax8.legend(fontsize=11)
ax8.grid(axis="y", alpha=0.3)
ax8.grid(axis="x", visible=False)

fig8.tight_layout()
add_tldr(fig8, f"TL;DR: Median ratio = {med_ratio:.2f}. {len(counts_below)}/{len(cat1_ratios)} cases favor DTV. Budget=6144.")
...
if cat3:
    ...
    ax_diag.set_title(f"[Internal] OLD: Cat3 Repair Loop Depth ({len(cat3)} DTV-fail cases)", fontsize=14, fontweight="bold")
    ...
    cat3_at_budget = sum(
        1 for c in cat3
        if cases[c]["dtv"]["total_tokens"] >= BUDGET * 0.95
    ...
    add_tldr(fig_diag,
        ...
        f"{cat3_at_budget}/{len(cat3)} near/at budget limit.")
    fig_diag.savefig(OUT_DIR / "fig_cat3_diagnosis.png", dpi=150, bbox_inches="tight")
    print("Saved fig_cat3_diagnosis.png")
else:
    print("Skipped fig_cat3_diagnosis.png (no Cat3 cases)")

# =====================================================================

print(f"\nAll figures saved to {OUT_DIR.resolve()}/")
print(f"\nDataset: OLD 100-case, budget=6144 fixed, do_sample=on")
print(f"Total paired cases: {N_TOTAL}")
print(f"Categories: Cat1={len(cat1_all)} (1a={len(cat1a)}, 1b={len(cat1b)}, 1c={len(cat1c)}, 1d={len(cat1d)}), "
      f"Cat2={len(cat2)}, Cat3={len(cat3)}, Cat4={len(cat4)}")
print(f"DTV pass: {dtv_total}/{N_TOTAL} = {dtv_pct:.1f}%")
print(f"Naive pass: {naive_total}/{N_TOTAL} = {naive_pct:.1f}%")
