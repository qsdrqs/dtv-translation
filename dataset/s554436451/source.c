...
bool func (ull x) {
	...
	ull mv = xy[mid].b;
	sll person = ((sll)(x % k) - b[mv]);
	if (res + smax(person * c[mv], person * d[mv]) >= 0) {
		// puts("!");
		return true;
	}
	...
	for (i = 0; i < mid; i++) {
		...
		sll maybe = res + ifu - (sll)(d[v] * (k - b[v]));
		sll dif = ((sll)(x % k) - b[v]);
		if (maybe + smax(dif * c[v], dif * d[v]) >= 0) return true;
	}
	sll ifl = c[mv] * (-b[mv]);
	for (i = mid + 1; i < n; i++) {
		ull v = xy[i].b;

		sll maybe = res + ifl - (sll)(c[v] * (-b[v]));
		sll dif = ((sll)(x % k) - b[v]);
		if (maybe + smax(dif * c[v], dif * d[v]) >= 0) return true;
	}
	return false;
