...
2. keep this file up to date (last updated: 2026-04-29)
...
- `run_delta.sh`: unified Slurm job launcher (Apptainer, `JOB_TIME_MIN` override); pass `--task js_ts|c_rust` (selects the python runner; the remote code dir is the canonical js2ts path).
- `sync_delta.sh`: rsync repo to deltaai (default) or delta x86; override host with `--host <ssh_alias>`, override remote dir with `--path <remote>`.
- `sync_delta_back.sh`: rsync remote `result/` back to local `result_delta/`; defaults to the js2ts remote, override with `--path <remote>`.
- `delta.md`: unified runbook covering both tasks.
- `*_old.{sh,md}` (e.g. `run_delta_js_ts_old.sh`, `sync_delta_*_old.sh`, `delta_*_old.md`): legacy per-task variants kept for reference, not used.

**delta and deltaai are different hosts: deltaai is the GH200 aarch64 cluster, delta is the x86_64 GPU cluster. Both can be used for experiments, but deltaai is the primary target for large-scale runs.**
