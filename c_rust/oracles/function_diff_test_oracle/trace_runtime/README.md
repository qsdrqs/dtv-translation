...
The runtime libraries provide helpers to build the JSON payloads without embedding
large JSON string literals in instrumentation code.

### C Helpers

- `DTV_JSON_ARRAY_DECLARE(name, cap)`
- `dtv_json_array_push_*` / `dtv_json_array_push_ptr_*`
- `dtv_json_array_finish`
- `dtv_json_value_*` / `dtv_json_value_ptr_*`

### Rust Helpers

- `DtvJsonArray`
- `json_value_*`, `json_value_ref_*`, `json_value_raw_ptr_*`, `json_value_slice_*`, `json_value_array_*`
